import { ImageResponse } from 'next/og';

export const runtime = 'edge';

// Generates a 1024x1024 image, which is the required dimension for App Store and Google Play Icons.
// Apple requires NO transparency and NO rounded corners in the source file.
export async function GET() {
    return new ImageResponse(
        (
            <div
                style={{
                    width: '100%',
                    height: '100%',
                    display: 'flex',
                    flexDirection: 'column',
                    alignItems: 'center',
                    justifyContent: 'center',
                    background: '#003366', // Primary Navy Blue
                }}
            >
                <svg
                    width="600" height="450" viewBox="0 0 160 120"
                    fill="none"
                >
                    {/* Reversed White Fill for Icon */}
                    <path d="M30 15 C 30 15, 30 10, 45 10 H 125 C 145 10, 150 20, 150 35 V 55 C 150 65, 145 70, 135 70 H 70 V 55 H 115 V 35 H 65 C 50 35, 50 45, 50 55 V 110 H 30 Z" fill="#ffffff" />
                    <path d="M55 80 H 135 C 145 80, 150 85, 150 95 V 115 H 55 Z" fill="#ffffff" />
                    <rect x="115" y="35" width="20" height="8" rx="4" fill="#003366" />
                </svg>
            </div>
        ),
        {
            width: 1024,
            height: 1024,
        }
    );
}
